			...
			for (int q = 0; q <= order; q++) {
				...
				if (!((p - Math.abs(q)) % 2 == 0) && (-p <= q) && (q <= p)) {
					for (int i = 0; i < insideDisc.length; i++) {
						...
						double temp = (double) (inputImage.getRGB(insideDisc.X_Index[i], insideDisc.Y_Index[i])
								/ 255.0);
					...
					if (lambdaChoise == 1) {
						lambda = 2 * (p + 1)
								/ (Math.PI * Math.pow(inputImage.getHeight() * inputImage.getWidth() - 1, 2));
					} else {
	...
	public static void main(String args[]) {
		...
		try {
			image = ImageIO.read(new File(
					"C:\\Users\\310176547\\Documents\\workspace-sts-3.6.2.RELEASE\\SolveIT\\images\\0\\subImage511.jpg"));
		} catch (Exception e) {
			e.printStackTrace();
		}
