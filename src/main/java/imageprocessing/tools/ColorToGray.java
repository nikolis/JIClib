...
public class ColorToGray {
	...
	/**
	 * Classical method turning a colored image in a grayscale image using the
	 * Average method downside:The pour representation of luminosity
	 * 
	 * @param originalImage
	 * @return
	 */
	public static BufferedImage toGrayAvg(BufferedImage originalImage) {
		...
		for (int i = 0; i < originalImage.getWidth(); i++) {
			for (int j = 0; j < originalImage.getHeight(); j++) {
				...
				newPixel = GeneralImagingOperations.colorToRGB(alpha, meanOfPixelIntesities, meanOfPixelIntesities,
						meanOfPixelIntesities);
	...
	/**
	 * luminance method plays off the fact that cone density in the human eye is
	 * not uniform across colors.Humans perceive green more strongly than red,
